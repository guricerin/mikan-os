...
// すべてのLOADセグメントを1つのかたまりとみなし、その先頭と末尾を求める
...
EFI_STATUS ReadFile(EFI_FILE_PROTOCOL* file, VOID** buffer) {
    // ファイル情報取得
    // sizeof(CHAR16) * 12 はEFI_FILE_INFO::FileName[]（可変長文字列）の領域分（\kernel.elfの11文字とヌル文字分）
...
/// ブートローダが格納されているブロックデバイス（起動メディア）に対応するBlock I/O Protocolを開く
/// image_handle : ブートローダへのハンドル（UEFIアプリケーションはイメージと呼ばれる）
EFI_STATUS OpenBlockIoProtocolForLoadedImage(EFI_HANDLE image_handle, EFI_BLOCK_IO_PROTOCOL** block_io) {
    ...
    status = gBS->OpenProtocol(
        loaded_image->DeviceHandle, // 記憶装置へのハンドラ
...
EFI_STATUS ReadBlocks(EFI_BLOCK_IO_PROTOCOL* block_io, UINT32 media_id, UINTN read_bytes, VOID** buffer) {
    EFI_STATUS status = gBS->AllocatePool(EfiLoaderData, read_bytes, buffer);
    if (EFI_ERROR(status)) {
        return status;
    }
...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE* system_table) {
    ...
    status = root_dir->Open(
        ...
        0);
    if (EFI_ERROR(status)) {
        Print(L"failed to open file '\\kernel.elg': %r\n", status);
        Halt();
    }
    ...
    // カーネルにファイルシステムを構築するため、ボリュームイメージをメモリに読み込む
    ...
    if (status == EFI_SUCCESS) {
        ...
        // fat_diskがなければ起動メディア全体を読み込む
        // UEFI BIOSのBlock I/O Protocolを利用する
        EFI_BLOCK_IO_PROTOCOL* block_io;
        status = OpenBlockIoProtocolForLoadedImage(image_handle, &block_io);
        if (EFI_ERROR(status)) {
            Print(L"failed to open Block I/O Protocol: %r", status);
            Halt();
        }
        ...
        // ブロックデバイスからvolume_imageにデータを読み込む
        status = ReadBlocks(block_io, media->MediaId, volume_bytes, &volume_image);
        if (EFI_ERROR(status)) {
            Print(L"failed to read blocks: %r", status);
            Halt();
        }
    }
    ...
    entry_point(&frame_buffer_config, &memmap, acpi_table, volume_image);
