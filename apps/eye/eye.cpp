...
void DrawEye(uint64_t layer_id__flags, int mouse_x, int mouse_y, uint32_t color) {
    ...
    // マウスカーソルが描画領域の外にある場合に、目玉が描画領域からはみ出ないようにする
    ...
    SyscallWinFillRectangle(layer_id__flags, eye_x - kEyeSize / 2, eye_y - kEyeSize / 2, kEyeSize, kEyeSize, color);
}
