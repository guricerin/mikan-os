...
std::tuple<bool, int> WaitEvent() {
    ...
    while (true) {
        ...
        if (e.type == AppEvent::kQuit) {
            return {true, 0};
        } else if (e.type == AppEvent::kKeyPush && e.arg.keypush.press) {
            return {false, e.arg.keypush.keycode};
        }
    }
}
...
bool UpdateStartLine(int* start_line, int height, size_t num_lines) {
    while (true) {
        ...
        if ((diff < 0 && *start_line == 0) || (diff > 0 && *start_line == num_lines - height)) { // 文頭 or 文末
            continue;
        }
...
extern "C" void main(int argc, char** argv) {
    auto print_help = [argv]() {
        fprintf(stderr, "Usage: %s [-w WIDTH] [-h HEIGHT] [-t TAB] <dile>\n", argv[0]);
    };
    ...
    const char* last_slash = strchr(filepath, '/');
