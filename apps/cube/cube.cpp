...
const int kScale = 50 /* ウィンドウと立方体のサイズに影響 */, kMargin = 10 /* 立方体と描画枠の余白の大きさ */;
// 描画領域
...
// 回転後の立方体の頂点座標
array<Vector3D<double>, kCube.size()> g_vert;
// 立方体の各面の重心のz座標
array<double, kSurface.size()> g_centerz4;
// 3次元空間で回転した立方体の各頂点を画面に射影したときの座標
array<Vector2D<int>, kCube.size()> g_scr;
...
extern "C" void main(int argc, char** argv) {
    ...
    while (true) {
        ...
        for (int i = 0; i < kCube.size(); i++) {
            ...
            g_vert[i].z = zt * yp - kScale * cv.x * ya;
            g_vert[i].x = xt * zp - yt * za;
            g_vert[i].y = yt * zp + xt * za;
        }
        ...
        for (int sur = 0; sur < kSurface.size(); ++sur) {
            ...
            for (int i = 0; i < kSurface[sur].size(); ++i) {
                g_centerz4[sur] += g_vert[kSurface[sur][i]].z;
            }
        }

        // 画面を一旦クリアし、立方体を描画
        SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW, 4, 24, kCanvasSize, kCanvasSize, 0);
...
void DrawObj(uint64_t layer_id) {
    // オブジェクト座標 g_vert を スクリーン座標 g_scr に変換（画面奥が Z+）
    for (int i = 0; i < kCube.size(); i++) {
        const double t = 6 * kScale / (g_vert[i].z + 8 * kScale);
        g_scr[i].x = (g_vert[i].x * t) + kCanvasSize / 2;
        g_scr[i].y = (g_vert[i].y * t) + kCanvasSize / 2;
    }

    while (true) {
        // 奥にある（= Z 座標が大きい）オブジェクトから順に描画
        double* const zmax = max_element(g_centerz4.begin(), g_centerz4.end());
        if (*zmax == numeric_limits<double>::lowest()) {
            break;
        }
        const int sur = zmax - g_centerz4.begin();
        g_centerz4[sur] = numeric_limits<double>::lowest();

        // 法線ベクトルがこっちを向いてる面だけ描画
        const auto v0 = g_vert[kSurface[sur][0]],
                   v1 = g_vert[kSurface[sur][1]],
                   v2 = g_vert[kSurface[sur][2]];
...
void DrawSurface(uint64_t layer_id, int sur) {
    ...
    for (int i = 0; i < surface.size(); i++) {
        const auto p0 = g_scr[surface[(i + 3) % 4]], p1 = g_scr[surface[i]];
        ymin = min(ymin, p1.y);
        ymax = max(ymax, p1.y);
        if (p0.y == p1.y) {
            continue;
