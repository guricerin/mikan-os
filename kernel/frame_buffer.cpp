...
namespace {
    int BytesPerPixel(const PixelFormat& format) {
        switch (format) {
...
Error FrameBuffer::Initailize(const FrameBufferConfig& config) {
    config_ = config;
    ...
    if (config_.frame_buffer) {
        ...
        buffer_.resize(
            bytes_per_pixel * config_.horizontal_resolution * config_.vertical_resolution);
