...
namespace usb::xhci {
    ...
    Error Device::ControlIn(EndpointID ep_id, SetupData setup_data,
        ...
        Log(LogLevel::Debug, "Device::ControlIn: ep addr %d, buf 0x%08x, len %d\n",
            ep_id.Address(), buf, len);
        if (ep_id.Number() < 0 || 15 < ep_id.Number()) {
            return MAKE_ERROR(ErrorCode::InvalidEndpointNumber);
        }
        ...
        if (tr == nullptr) {
            return MAKE_ERROR(ErrorCode::TransferRingNotSet);
        }
    ...
    Error Device::ControlOut(EndpointID ep_id, SetupData setup_data,
        ...
        Log(LogLevel::Debug, "Device::ControlOut: ep addr %d, buf 0x%08x, len %d\n",
            ep_id.Address(), buf, len);
        if (ep_id.Number() < 0 || 15 < ep_id.Number()) {
            return MAKE_ERROR(ErrorCode::InvalidEndpointNumber);
        }
    ...
    Error Device::InterruptOut(EndpointID ep_id, void* buf, int len) {
        ...
        Log(LogLevel::Debug, "Device::InterrutpOut: ep addr %d, buf %08lx, len %d, dev %08lx\n",
    ...
    Error Device::OnTransferEventReceived(const TransferEventTRB& trb) {
        ...
        if (trb.bits.completion_code != 1 /* Success */ &&
            ...
            return MAKE_ERROR(ErrorCode::TransferFailed);
        }
        Log(LogLevel::Debug, trb);
        ...
        if (!opt_setup_stage_trb) {
            Log(LogLevel::Debug, "No Corresponding Setup Stage for issuer %s\n",
                kTRBTypeToName[issuer_trb->bits.trb_type]);
            if (auto data_trb = TRBDynamicCast<DataStageTRB>(issuer_trb)) {
                Log(LogLevel::Debug, *data_trb);
            }
            return MAKE_ERROR(ErrorCode::NoCorrespondingSetupStage);
        }
